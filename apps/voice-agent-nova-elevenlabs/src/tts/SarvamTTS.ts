...
export class SarvamTTS {
  ...
  /**
   * Sarvam hi-IN requires at least one character from the target language
   * (Devanagari) in the overall turn. Pure English text returns 400.
   * We inject a minimal Devanagari token ONCE per turn (not per chunk)
   * to satisfy the API without audible repetition.
   */
  private static readonly DEVANAGARI_RE = /[\u0900-\u097F]/;
  /** Whether any Devanagari text has been sent in the current turn. */
  private hindiInjectedThisTurn = false;

  private ensureHindiCompat(text: string): string {
    // If text already has Devanagari, mark and return as-is.
    if (SarvamTTS.DEVANAGARI_RE.test(text)) {
      this.hindiInjectedThisTurn = true;
      return text;
    }
    // If we've already sent Devanagari this turn, no need to inject again.
    if (this.hindiInjectedThisTurn) return text;
    // First pure-English chunk this turn: prepend a short Hindi connector.
    this.hindiInjectedThisTurn = true;
    return 'तो, ' + text;
  }

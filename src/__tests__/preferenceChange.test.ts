 ...
 *  5. Latest value reflected in getStateSnapshot
 */
...
} from '../modules/conversation/PreferenceChangeDetector';
...
describe('detectPreferenceChanges', () => {
  ...
  describe('Budget detection', () => {
    ...
    it('detects budget correction with "actually"', () => {
      const result = detectPreferenceChanges(
        'Actually budget 1 crore hai mera',
        null,
        '80 lakh',
      );
      expect(result.hasCorrection).toBe(true);
    ...
    it('detects crore amounts', () => {
      const result = detectPreferenceChanges('Actually budget 1.5 crore hai', null, '80 lakh');
      expect(result.newBudget).toContain('1.5');
    });
  });
...
describe('SessionState preference tracking', () => {
  ...
  describe('BHK preference', () => {
    ...
    it('reflects latest BHK in state snapshot', () => {
      ...
      expect(snap.bhkPreference).toBe('3 BHK');
    });

    it('reflects latest BHK in prompt block', () => {
      session.extractFromUserTranscript('2 BHK chahiye');
      session.extractFromUserTranscript('Nahi, actually 3 BHK chahiye');
      const block = session.toPromptBlock();
      expect(block).toContain('3 BHK');
      expect(block).not.toContain('2 BHK');
    });
  });
  ...
  describe('Budget preference', () => {
    ...
    it('reflects latest budget in state snapshot', () => {
      session.extractFromUserTranscript('Budget 80 lakh hai');
      session.extractFromUserTranscript('Actually budget 1 crore hai mera');
      const snap = session.getStateSnapshot();
      expect(String(snap.budgetMentioned)).toContain('1');
    });

    it('reflects latest budget in prompt block', () => {
      session.extractFromUserTranscript('Budget 80 lakh hai');
      session.extractFromUserTranscript('Nahi, budget updated hai — 1 crore hai');
      const block = session.toPromptBlock();
  ...
  describe('prompt block', () => {
    it('shows BHK preference when set', () => {
      session.extractFromUserTranscript('3 BHK chahiye');
      const block = session.toPromptBlock();
      expect(block).toContain('BHK preference: 3 BHK ✓');
    });

    it('shows budget when set', () => {
      session.extractFromUserTranscript('Budget 80 lakh hai');
      const block = session.toPromptBlock();
      expect(block).toContain('Budget:');
    ...
    it('shows "not specified" when BHK not set', () => {
      const block = session.toPromptBlock();
      expect(block).toContain('BHK preference: not specified');
    });

    it('shows "not specified" when budget not set', () => {
      const block = session.toPromptBlock();
      expect(block).toContain('Budget: not specified');
    });
  });
});
